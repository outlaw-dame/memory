...
import { Form, useNotify, useTranslate, useGetIdentity, useRedirect, useDataProvider, SelectInput } from 'react-admin'
...
const PostBlock = ({ inReplyTo, mention }) => {
  ...
  const mentionables = useMemo(
    () =>
      sortBy(uniqBy([...followers, ...following], 'id'), 'preferredUsername').map(actor => {
  ...
  */
  const processEditorContent = useCallback(content => {
    ...
    return {
      processedContent: paragraph ? paragraph.innerHTML.replace(/<br\s*\/?>/gi, '\n').trim() : '',
      mentionedUsersUris
    }
  }, [])

  const getFormattedMentions = useCallback(mentionedUsersUris => {
    return mentionedUsersUris
      .map(uri => {
            ...
            editorOptions={{
              ...
              extensions: [
                ...
                Placeholder.configure({
                  placeholder: inReplyTo ? translate('app.input.reply') : translate('app.input.message')
                }),
                Mention.configure({
                  HTMLAttributes: {
                    class: 'mention'
