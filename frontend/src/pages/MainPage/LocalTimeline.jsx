...
const LocalTimeline = () => {
  ...
  const curPage = ctx.call('public-posts.get', { initTime: initTime, nextPage: initTime });
  const activities = curPage.hits.hits.map((hit) => {
    return { type: "Activity", hit._source.actorUri, hit._source.activityUri }
  });
  /*
  const {
    items: activities,
