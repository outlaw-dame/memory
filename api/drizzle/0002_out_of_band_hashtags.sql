ALTER TABLE "posts" ADD COLUMN IF NOT EXISTS "hashtags" text[] NOT NULL DEFAULT '{}'::text[];
...
SET "hashtags" = COALESCE(
  (
    SELECT ARRAY(
      ...
      FROM regexp_matches("posts"."content", '#([[:alnum:]_]{1,64})', 'g') AS match
    )
  ),
  ARRAY[]::text[]
)
...
CREATE VIEW "public"."unified_feed_view" AS (
  ...
  SELECT
    ...
    at_posts.title,
    at_posts.summary,
    COALESCE(at_posts.canonical_url, at_posts.at_uri) as canonical_url,
    at_posts.created_at,
    at_posts.is_public,
